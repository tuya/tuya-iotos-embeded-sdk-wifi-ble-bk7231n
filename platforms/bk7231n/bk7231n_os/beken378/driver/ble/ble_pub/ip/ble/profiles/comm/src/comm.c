...
static uint8_t bk_ble_service_init (struct prf_task_env* env, uint16_t* start_hdl, uint16_t app_task, uint8_t sec_lvl,  struct bk_ble_db_cfg* params)
{
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        *start_hdl = shdl;
        ...
        env->desc.idx_max           = BLE_CONN_IDX_MAX;
        env->desc.state             = ble_env->state;
        env->desc.default_handler   = &bk_ble_default_handler;

        bk_printf("ble_env->start_hdl = 0x%x",ble_env->start_hdl);
