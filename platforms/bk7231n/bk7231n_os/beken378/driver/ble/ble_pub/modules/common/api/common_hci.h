...
enum hci_opcode
{
    ...
    HCI_VS_ADV_TEST_MODE_START_CMD_OPCODE       = 0xFC56,
...
enum hci_evt_code
{
    ...
    #endif // (BLE_2MBPS)
    HCI_VS_SET_PREF_SLAVE_LATENCY_CMP_EVT_SUBCODE  = 0x0D,
    /// DBG Events Subcodes
...
struct hci_vs_set_pref_slave_latency_cmd_cmp_evt
{
    /// status
    uint8_t status;
    /// Connection handle
...
/// HCI Vendor specific to start Advertising test mode - vendor specific
/// The transmitted advertising packet is non connectable and non scannable
struct hci_vs_adv_test_mode_start_cmd
{
    /// Advertising interval (1 step = 625us)
    uint16_t interval;

    /// Advertising channel map (bit 0 - 37, bit 1 - 38, bit 2 - 39)
    uint8_t chmap;

    /// Number of transmission of advertising packet to perform per enabled advertising channel.
    uint8_t nb_tx;

    /// Random address to transmit, if zero, Public address is used.
    uint8_t  rnd_addr[6];

    /// Length of advertising data to transmit (range [0:31])
    uint8_t data_len;

    /// Advertising data to transmit
    uint8_t data[31];
};
